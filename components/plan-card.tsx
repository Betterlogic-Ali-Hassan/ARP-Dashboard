...
import { type PlanType, type BillingPeriodType, PLANS, getPreviousPlan } from "@/lib/plan-utils"
import { cn } from "@/lib/utils"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"

interface PlanCardProps {
  plan: PlanType
  status: "active" | "canceled" | "scheduled_downgrade" | "past_due"
...
export function PlanCard({
  ...
  const amount = billingPeriod === "yearly" ? planDetails.pricing.yearly : planDetails.pricing.monthly
  ...
  const getDowngradeButtonClasses = () => {
    if (!previousPlanDetails) return "border-gray-300 text-gray-400 cursor-not-allowed"

    // Extract color name from the primary color class (e.g., "bg-amber-500" -> "amber")
    const colorName = previousPlanDetails.color.primary.split("bg-")[1].split("-")[0]
  ...
  const { name: colorName, shade: colorShade } = extractColorInfo(planDetails.color.primary)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Your current subscription plan and usage details.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid gap-6">
          ...
          <div className="mt-4 space-y-2">
            ...
            <Progress
              ...
              className="h-2"
              indicatorClassName={
                devicesUsed / deviceLimit > 0.9
                  ? "bg-red-500"
                  : devicesUsed / deviceLimit > 0.7
                    ? "bg-amber-500"
                    : "bg-emerald-500"
              }
            />
            ...
            <div className="bg-gradient-to-r from-amber-50 to-amber-100 dark:from-amber-950/40 dark:to-amber-900/40 border border-amber-200 dark:border-amber-800 rounded-lg p-4 relative overflow-hidden">
              <div className="flex items-start gap-3 relative z-10">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-amber-700 dark:text-amber-400 text-sm">
                    Your plan will be downgraded on <span className="font-medium">{nextBillingDate}</span>.
                  </p>
                  <p className="text-amber-700 dark:text-amber-400 text-sm mt-1">
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-3">
        {status === "active" ? (
          <>
            <Button
              variant="default"
              className={cn("w-full sm:w-auto change-plan-btn", planDetails.color.primary, hoverClass)}
              style={buttonStyle}
              onClick={onUpgrade}
            >
              Change Plan
          ...
          <Button
            variant="default"
            className={cn("w-full sm:w-auto", planDetails.color.primary, hoverClass)}
            style={buttonStyle}
