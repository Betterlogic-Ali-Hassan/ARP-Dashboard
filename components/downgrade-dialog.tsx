...
export function DowngradeDialog({
  ...
  const [downgradeType, setDowngradeType] = useState<"immediate" | "scheduled">(
    "scheduled"
  );
  ...
  const calculateSavings = () => {
    ...
    const currentPrice =
      billingPeriod === "yearly"
        ? Number.parseFloat(
            currentPlanDetails.pricing.yearlyPerMonth.replace("$", "")
          )
        : Number.parseFloat(
            currentPlanDetails.pricing.monthly.replace("$", "")
          );

    const targetPrice =
      billingPeriod === "yearly"
        ? Number.parseFloat(
            targetPlanDetails.pricing.yearlyPerMonth.replace("$", "")
          )
  ...
  const { lostFeatures, keptFeatures } = useMemo(() => {
    ...
    const keptFeatures = currentFeatures.filter((feature) =>
  ...
  const excludedKeptFeatures = [
    "Team collaboration features",
    "Advanced analytics",
  ];

  const filteredKeptFeatures = useMemo(() => {
    return keptFeatures.filter(
      (feature) => !excludedKeptFeatures.includes(feature)
    );
  }, [keptFeatures]);
  ...
  const handleSubmit = async () => {
    if (deviceLimitExceeded) {
      setError(
        `You need to deactivate at least ${excessDevices} ${
          excessDevices === 1 ? "device" : "devices"
        } before downgrading to the ${targetPlanDetails.name} plan.`
      );
      return;
    }
                  ...
                  <div className='space-y-4'>
                    ...
                    <div className='relative'>
                      ...
                      <div className='relative z-10 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700'>
                        <div
                          className={`${currentPlanDetails.color.primary} p-3`}
                        >
                          <Badge
                            variant='secondary'
                        ...
                        <div className='bg-white dark:bg-gray-900 p-4 border border-gray-100 dark:border-gray-800'>
                          <div className='flex justify-between items-center'>
                            ...
                            <span
                              className={`font-bold ${currentPlanDetails.color.badgeText}`}
                            >
                      ...
                      <div className='relative z-10 rounded-lg shadow-sm overflow-hidden'>
                        <div
                          className={`${targetPlanDetails.color.primary} p-3`}
                        >
                          <Badge
                            variant='secondary'
                            className='bg-white/20 text-white border-0 backdrop-blur-sm'
                        ...
                        <div className='bg-white dark:bg-gray-900 p-4 border border-gray-100 dark:border-gray-800'>
                          <div className='flex justify-between items-center'>
                            ...
                            <span
                              className={`font-bold ${targetPlanDetails.color.badgeText}`}
                            >
                        ...
                        <div className='flex items-center justify-between'>
                          ...
                          <div className='flex items-center gap-2'>
                            <span className='text-amber-700 dark:text-amber-400'>
                              {currentPlanDetails.deviceLimit} →{" "}
                              {targetPlanDetails.deviceLimit}
                            </span>
                          </div>
                        </div>
                      </div>
                              ...
                              <div className='ml-3 flex-1'>
                                ...
                                <div className='mt-2 text-xs bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 p-2 rounded'>
                                  <div className='flex'>
                                    ...
                                    <span>
                                      Recommended: Keep all features until your
                                      billing date
                                    </span>
                                  </div>
                                </div>
                          ...
                          <div className='p-4 bg-white dark:bg-gray-800'>
                            <div className='flex items-start'>
                              ...
                              <div className='ml-3 flex-1'>
                                ...
                                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
                                  Your plan will be downgraded now with a
                                  prorated refund
                                </p>
                              </div>
                            </div>
                      ...
                      >
                        ...
                        <AlertDescription className='flex flex-col gap-2'>
                          ...
                          <p className='text-sm'>
                            ...
                            , but the {targetPlanDetails.name} plan only
                            supports up to{" "}
                            <span className='font-semibold'>
                              {targetPlanDetails.deviceLimit} devices
                            </span>
                            .
                          </p>
                          <p className='text-sm'>
            ...
            <div className='p-6'>
              <div className='flex flex-col items-center justify-center text-center p-6'>
                ...
                <p className='text-gray-500 dark:text-gray-400 mb-6 max-w-md'>
                  You're currently on our Basic (Free) plan, which is our lowest
                  tier. There are no plans available to downgrade to.
                </p>
