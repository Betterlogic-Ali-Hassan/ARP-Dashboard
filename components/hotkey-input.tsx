...
export function HotkeyInput({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (modifiers.length === 0) {
      onError?.("Please include at least one modifier key (Ctrl, Alt, Shift, or Command)")
      return
    }
  ...
  return (
    <div className="relative">
      <Input
        ...
        disabled={disabled}
        readOnly
        className={cn("pr-10", isRecording && "border-green-500 ring-1 ring-green-500", disabled && "opacity-60")}
      />
      <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
        <Keyboard className={cn("h-4 w-4 text-muted-foreground", isRecording && "text-green-500")} />
      </div>
    </div>
  )
}
