...
import { Search, Filter, X, Check, PlusCircle, ChevronRight } from "lucide-react"
import { Sidebar } from "@/components/sidebar"
import { TicketList } from "@/components/ticket-list"
import TicketChatPanel from "@/components/ticket-chat-panel"
import { CreateTicketDialog } from "@/components/create-ticket-dialog"
import { cn } from "@/lib/utils"
import { useTicketStore, type TicketStatus, type TicketPriority, type TicketCategory } from "@/lib/ticket-store"
...
export default function SupportPage() {
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    // Category filter
    const matchesCategory = categoryFilters.length === 0 || categoryFilters.includes(ticket.category)

    return matchesSearch && matchesStatus && matchesPriority && matchesCategory
  })

  // Count active filters
  const activeFilterCount = statusFilters.length + priorityFilters.length + categoryFilters.length
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main
          className={cn(
            ...
            !isMobile && isChatPanelOpen ? "max-w-[50%] lg:max-w-[60%] xl:max-w-[65%]" : "max-w-full",
          )}
        >
          <div className="mx-auto max-w-4xl">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-3 mb-3 sm:mb-4 md:mb-6">
              <div>
                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight mb-0.5 sm:mb-1">Support</h1>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-2.5 sm:p-3 md:p-4 mb-3 sm:mb-4 md:mb-6 shadow-sm">
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <Popover open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                  ...
                  <PopoverContent className="w-[260px] sm:w-[280px] md:w-80" align="end">
                    <div className="space-y-3 sm:space-y-4">
                      <div className="flex items-center justify-between">
                        <h3 className="font-medium text-sm sm:text-base">Filter Tickets</h3>
                        {activeFilterCount > 0 && (
                          <Button
                            variant="ghost"
                            size="sm"
                      ...
                      <div className="space-y-1.5 sm:space-y-2">
                        <h4 className="text-xs sm:text-sm font-medium">Status</h4>
                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                          {[
                            ...
                            <div key={status.value} className="flex items-center space-x-1.5 sm:space-x-2">
                              <Checkbox
                                id={`status-${status.value}`}
                                checked={statusFilters.includes(status.value as TicketStatus)}
                                onCheckedChange={() => toggleStatusFilter(status.value as TicketStatus)}
                                className="h-3.5 w-3.5 sm:h-4 sm:w-4"
                              />
                              <Label htmlFor={`status-${status.value}`} className="text-xs sm:text-sm cursor-pointer">
                                {status.label}
                              </Label>
                            </div>
                          ))}
                      ...
                      <div className="space-y-1.5 sm:space-y-2">
                        <h4 className="text-xs sm:text-sm font-medium">Priority</h4>
                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                          {[
                            { value: "low", label: "Low" },
                            ...
                            <div key={priority.value} className="flex items-center space-x-1.5 sm:space-x-2">
                              <Checkbox
                                id={`priority-${priority.value}`}
                                checked={priorityFilters.includes(priority.value as TicketPriority)}
                                onCheckedChange={() => togglePriorityFilter(priority.value as TicketPriority)}
                                className="h-3.5 w-3.5 sm:h-4 sm:w-4"
                              />
                              <Label
                      ...
                      <div className="space-y-1.5 sm:space-y-2">
                        <h4 className="text-xs sm:text-sm font-medium">Category</h4>
                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                          {[
                            ...
                            <div key={category.value} className="flex items-center space-x-1.5 sm:space-x-2">
                              <Checkbox
                                id={`category-${category.value}`}
                                checked={categoryFilters.includes(category.value as TicketCategory)}
                                onCheckedChange={() => toggleCategoryFilter(category.value as TicketCategory)}
                                className="h-3.5 w-3.5 sm:h-4 sm:w-4"
                              />
                              <Label
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
              <div className="bg-white dark:bg-gray-900 p-3 sm:p-4 md:p-6 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-base sm:text-lg md:text-xl font-semibold mb-1 sm:mb-2">Knowledge Base</h2>
                ...
                <Button variant="outline" className="w-full group text-xs sm:text-sm h-8 sm:h-9 md:h-10">
                  Browse Articles
              ...
              <div className="bg-white dark:bg-gray-900 p-3 sm:p-4 md:p-6 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-base sm:text-lg md:text-xl font-semibold mb-1 sm:mb-2">Community Forum</h2>
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-3 sm:mb-4">
