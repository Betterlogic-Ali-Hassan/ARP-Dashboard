...
const PREDEFINED_SOUNDS = [
  { id: "default", name: "Default Notification", url: "/notification-sound.mp3" },
...
const RefreshCountdown = ({
  ...
  return (
    <span className="text-[10px] text-muted-foreground">
      Next: {Math.floor(countdown / 60) > 0 ? `${Math.floor(countdown / 60)}m ` : ""}
      {countdown % 60}s
    </span>
  )
}
...
const generateRandomNotification = (): Notification => {
  ...
  const keywords = ["In Stock", "Available", "Sale", "New Price", "Limited Time", "Deal", "Discount"]

  const eventTypes: Array<"Keyword Found" | "Keyword Lost" | "Page Change Detected"> = [
    "Keyword Found",
    "Keyword Lost",
  ...
  const deviceNames = ["Windows PC", "MacBook Pro", "iPad", "iPhone 13", "Galaxy S22", "Office PC"]

  const profileNames = ["Default Profile", "Work Profile", "Shopping Profile", "Personal Profile", "Media Profile"]
...
const SoundSettingsDialog = ({
    ...
    if (isPlaying) {
      ...
      const soundToPlay =
        selectedSound === "custom" && customSoundUrl
          ? customSoundUrl
          : PREDEFINED_SOUNDS.find((s) => s.id === selectedSound)?.url || PREDEFINED_SOUNDS[0].url
  ...
  const validateAudioUrl = () => {
    ...
    audio.addEventListener("error", () => {
      setIsValidatingUrl(false)
      setUrlValidationStatus("invalid")
      setUrlValidationMessage("Invalid audio URL. Please check the URL and try again.")
    })
  ...
  return (
    <DialogContent className="sm:max-w-[500px]">
      <DialogHeader>
        ...
        <DialogDescription>
          Customize how notification sounds are played when new alerts are received.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-6 py-4">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">Enable or disable notification sounds</p>
          </div>
          <Button
        ...
        <div className="space-y-4">
          ...
          <RadioGroup value={selectedSound} onValueChange={setSelectedSound} className="space-y-3">
            {PREDEFINED_SOUNDS.map((sound) => (
              <div key={sound.id} className="flex items-center space-x-2">
                ...
                <Label htmlFor={sound.id} className="flex items-center">
                  {sound.name}
                  {sound.id === "default" && <Badge className="ml-2">Default</Badge>}
                </Label>
                <Button
                  variant="ghost"
                  size="icon"
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                {isPlaying && selectedSound === "custom" ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
              </Button>
            </div>
          </RadioGroup>
        </div>
        ...
        <Button
          ...
          disabled={!soundEnabled || (selectedSound === "custom" && !customSoundUrl)}
          className="w-full"
        >
          {isPlaying ? (
            <>
...
const ExportDialog = ({
  ...
  const [exportFilename, setExportFilename] = useState(`notifications-export-${new Date().toISOString().split("T")[0]}`)
  ...
  const filteredNotifications = useMemo(() => {
    ...
    if (activeTab !== "all") {
      result = result.filter((notification) => notification.eventType.toLowerCase() === activeTab)
    }
    ...
    if (selectedProfiles.length > 0) {
      ...
      result = result.filter((notification) => profileNames.some((name) => notification.profileName === name))
    }
    ...
    if (selectedDevices.length > 0) {
      ...
      result = result.filter((notification) => deviceNames.some((name) => notification.deviceName === name))
    }
    ...
    return result
  }, [notifications, activeTab, searchQuery, selectedProfiles, selectedDevices, readFilter, profiles, devices])
  ...
  const finishExport = () => {
    try {
      const exportData = JSON.parse(sessionStorage.getItem("exportData") || "[]")
  ...
  return (
    <DialogContent className="sm:max-w-[500px]">
      <DialogHeader>
        ...
        <DialogDescription>
          Export your notifications to CSV or JSON format with your preferred settings.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-6 py-4">
        ...
        <div className="bg-muted p-4 rounded-md space-y-2">
          ...
          <div className="text-sm space-y-1">
            ...
            <p>
              Notifications: <span className="font-medium">{filteredNotifications.length}</span>
            </p>
            <p>
              Filters Applied:{" "}
      ...
      <DialogFooter>
        <Button variant="outline" onClick={() => handleDialogOpen(false)} disabled={isExporting}>
          Cancel
        </Button>
        <Button onClick={exportNotifications} disabled={isExporting || filteredNotifications.length === 0}>
          {isExporting ? (
            <>
...
export default function NotificationCenter() {
  const { notifications, addNotification, removeNotification, clearAllNotifications, markAsRead, markAllAsRead } =
  ...
  const filteredDevices = useMemo(() => {
    ...
    return devices.filter((device) => selectedProfiles.includes(device.profileId))
  ...
  useEffect(() => {
    ...
    if (selectedSound === "custom" && customSoundUrl) {
      ...
      const sound = PREDEFINED_SOUNDS.find((s) => s.id === selectedSound)
      notificationSoundRef.current = new Audio(sound?.url || PREDEFINED_SOUNDS[0].url)
    }
    ...
    return () => {
      if (notificationSoundRef.current && "pause" in notificationSoundRef.current) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchNewNotifications, refreshInterval * 1000)

    // Clean up
    return () => clearInterval(intervalId)
  }, [addNotification, refreshInterval, soundEnabled, notificationSoundRef, selectedSound, customSoundUrl, volume])
  ...
  const hasActiveFilters =
    searchQuery || selectedProfiles.length > 0 || selectedDevices.length > 0 || readFilter !== "all"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1.5">
            View and manage all your recent notifications triggered by keyword activity across your devices.
          </p>
        </div>
      </div>
                ...
                <DropdownMenu>
                      ...
                      <span className="truncate">
                        {selectedProfiles.length === 0
                          ...
                          : selectedProfiles.length === 1
                            ? profiles.find((p) => p.id === selectedProfiles[0])?.name || "1 Profile"
                  ...
                  <DropdownMenuContent className="w-56">
                    ...
                    {profiles.map((profile) => (
                      ...
                      >
                        {profile.name}
                        {profile.isDefault && <span className="ml-2 text-xs text-muted-foreground">(Default)</span>}
                      </DropdownMenuCheckboxItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
                <TabsList className="h-auto p-1 bg-muted/60">
                  ...
                  <TabsTrigger value="keyword found" className="h-9 px-4 rounded-md">
                    <span className="flex items-center gap-1.5">
                      <Check className="h-4 w-4" />
                      <span>Found</span>
                      {notifications.filter((n) => n.eventType.toLowerCase() === "keyword found").length > 0 && (
                        <Badge
                          variant="secondary"
                          className="ml-1 h-5 px-1.5 min-w-[20px] flex items-center justify-center"
                        >
                          {notifications.filter((n) => n.eventType.toLowerCase() === "keyword found").length}
                        </Badge>
                      )}
                    </span>
                  </TabsTrigger>
                  <TabsTrigger value="keyword lost" className="h-9 px-4 rounded-md">
                    <span className="flex items-center gap-1.5">
                      <X className="h-4 w-4" />
                      <span>Lost</span>
                      {notifications.filter((n) => n.eventType.toLowerCase() === "keyword lost").length > 0 && (
                        <Badge
                          variant="secondary"
                          className="ml-1 h-5 px-1.5 min-w-[20px] flex items-center justify-center"
                        >
                          {notifications.filter((n) => n.eventType.toLowerCase() === "keyword lost").length}
                        </Badge>
                      )}
                    </span>
                  </TabsTrigger>
                  <TabsTrigger value="page change detected" className="h-9 px-4 rounded-md">
                    <span className="flex items-center gap-1.5">
                      <RefreshCw className="h-4 w-4" />
                      <span>Changes</span>
                      {notifications.filter((n) => n.eventType.toLowerCase() === "page change detected").length > 0 && (
                        <Badge
                          variant="secondary"
                          className="ml-1 h-5 px-1.5 min-w-[20px] flex items-center justify-center"
                        >
                          {notifications.filter((n) => n.eventType.toLowerCase() === "page change detected").length}
                        </Badge>
                      )}
                    </span>
                  </TabsTrigger>
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                ...
                <div className="flex items-center gap-2 bg-gradient-to-r from-muted/50 to-muted/30 p-1.5 rounded-md border border-muted/50 shadow-sm">
                  <div className="flex items-center">
                    <Dialog open={soundSettingsDialogOpen} onOpenChange={setSoundSettingsDialogOpen}>
                      <DialogTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          title={soundEnabled ? "Sound settings (enabled)" : "Sound settings (disabled)"}
                          className={cn(
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            className={cn("h-8 w-8 rounded-md transition-all", isRefreshing && "bg-muted/60")}
                          >
                ...
                <div className="flex items-center gap-2 bg-muted/40 p-1 rounded-md">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {unreadCount > 0 ? (
                        <DropdownMenuItem onClick={handleMarkAllAsRead} disabled={isRefreshing || unreadCount === 0}>
                          ...
                          <DropdownMenuSubContent>
                            ...
                            <DropdownMenuItem
                              onClick={() => {
                                ...
                                const json = JSON.stringify(filteredNotifications, null, 2)
              ...
              <div className="relative">
                ...
                <div className="space-y-6">
                  {filteredNotifications.map((notification, index) => {
                    const eventColor = getEventTypeColor(notification.eventType)
                    ...
                    return (
                      <div
                        key={notification.id}
                        className={cn("relative transition-all duration-500", isNew && !isRead && "animate-fadeIn")}
                      >
                        {/* Desktop timestamp */}
                        ...
                        <div
                          ...
                          style={{ backgroundColor: isRead ? "#9ca3af" : eventColor }}
                        ></div>
                                ...
                                <div className="flex-shrink-0 mt-1">
                                  <div
                                    className={cn(
                                      ...
                                      isRead ? "opacity-70" : isNew && "animate-pulse",
                                    )}
                                    style={{ backgroundColor: isRead ? "#9ca3af20" : `${eventColor}20` }}
                                  >
                                    {notification.eventType.toLowerCase() === "keyword found" && (
                                      <Check className="h-4 w-4" style={{ color: isRead ? "#9ca3af" : eventColor }} />
                                    )}
                                    {notification.eventType.toLowerCase() === "keyword lost" && (
                                      <X className="h-4 w-4" style={{ color: isRead ? "#9ca3af" : eventColor }} />
                                    )}
                                    {notification.eventType.toLowerCase() === "page change detected" && (
                                      <RefreshCw
                                        className="h-4 w-4"
                                        style={{ color: isRead ? "#9ca3af" : eventColor }}
                                      />
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center space-x-2 max-w-[70%]">
                                      <Globe
                                        className={cn(
                                          ...
                                          isRead ? "text-gray-400" : "text-muted-foreground",
                                        )}
                                      />
                                      <h3
                                        className={cn("truncate", isRead ? "font-normal text-gray-500" : "font-medium")}
                                      >
                                        {notification.url}
                                      </h3>
                                    </div>
                                  ...
                                  <div className="mb-3">
                                    <div className="flex flex-wrap items-center gap-2 mb-2">
                                      <Badge
                                        variant="outline"
                                        className={cn("bg-gray-50 dark:bg-gray-800", isRead && "opacity-70")}
                                      >
                                        {notification.type}
                                      </Badge>
                                      <div className="flex items-center">
                                  ...
                                  <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground">
                                    <div className="flex items-center gap-1.5">
                                      <Laptop className={cn("h-3.5 w-3.5", isRead && "opacity-70")} />
                                      <span className={cn("truncate", isRead && "text-gray-400")}>
                                        {notification.deviceName}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                      <Settings className={cn("h-3.5 w-3.5", isRead && "opacity-70")} />
                                      <span className={cn("truncate", isRead && "text-gray-400")}>
