...
export default function NotificationSettingsPage() {
  ...
  const [webhooks, setWebhooks] = useState([
    { id: 1, name: "Google Sheet Hook", service: "Custom URL", url: "https://example.com/success", enabled: true },
    { id: 2, name: "Telegram Bot", service: "Telegram", url: "https://example.com/fail", enabled: true },
  ])
  ...
  const [webhookMessage, setWebhookMessage] = useState({ type: "", message: "" })
  ...
  const [audioUrlValidationStatus, setAudioUrlValidationStatus] = useState<"idle" | "validating" | "valid" | "invalid">(
    "idle",
  )
  const [audioUrlValidationMessage, setAudioUrlValidationMessage] = useState("")
  ...
  const validateAudioUrl = (url: string) => {
    ...
    const hasValidExtension = fileExtension && validAudioFormats.includes(fileExtension)
    ...
    audioUrlValidationTimeoutRef.current = setTimeout(() => {
      if (!hasValidExtension) {
        ...
        setAudioUrlValidationMessage(`Invalid audio format. Supported formats: ${validAudioFormats.join(", ")}`)
      } else if (url.includes("error") || url.includes("fail")) {
        // Simulate failure for URLs containing "error" or "fail"
        setAudioUrlValidationStatus("invalid")
        setAudioUrlValidationMessage("Unable to access this audio file. Please check the URL and try again.")
      } else {
  ...
  const handleVerifyEmail = () => {
    if (!email || !email.includes("@")) {
      setEmailMessage({ type: "error", message: "Please enter a valid email address." })
      return
    }

    setShowOtpInput(true)
    setEmailMessage({ type: "info", message: "Verification code sent to your email. Please check your inbox." })
  }
  ...
  const verifyOtp = (otpValue: string) => {
    if (otpValue === "111111") {
      ...
      setEmailMessage({ type: "success", message: "Email added successfully for notifications" })
      setShowOtpInput(false)
    } else if (otpValue === "222222") {
      setEmailMessage({ type: "error", message: "Verification failed. Please try again." })
      setOtp(["", "", "", "", "", ""])
    } else {
      setEmailMessage({ type: "error", message: "Invalid OTP. Please try again." })
      setOtp(["", "", "", "", "", ""])
    }
  }
  ...
  const handleTestWebhook = (url: string) => {
    if (url.includes("/success")) {
      setWebhookMessage({ type: "success", message: "Webhook connection successful." })
    } else if (url.includes("/fail")) {
      setWebhookMessage({ type: "error", message: "Webhook test failed. Please check the URL and try again." })
    } else {
  ...
  const validateWebhookUrl = (url: string) => {
    ...
    urlValidationTimeoutRef.current = setTimeout(() => {
      if (url.includes("/success")) {
        ...
        setUrlValidationMessage("URL is not responding. Please check and try again.")
      } else {
        setUrlValidationStatus("invalid")
        setUrlValidationMessage("Invalid URL format. Please enter a valid webhook URL.")
      }
    }, 1500) // Simulate network delay
  }
  ...
  const handleWebhookSubmit = () => {
      ...
      setWebhooks(
        webhooks.map((webhook) =>
          webhook.id === editingWebhook
            ? { ...webhook, name: webhookName, service: webhookService, url: webhookUrl }
            : webhook,
        ),
      )
    } else {
    ...
    setWebhookMessage({
      type: "success",
      message: `Webhook ${editingWebhook !== null ? "updated" : "added"} successfully!`,
    })
  ...
  const confirmDeleteWebhook = () => {
    if (webhookToDelete) {
      ...
      setWebhookMessage({ type: "success", message: "Webhook deleted successfully!" })
  ...
  const handleToggleWebhook = (id: number) => {
    setWebhooks(webhooks.map((webhook) => (webhook.id === id ? { ...webhook, enabled: !webhook.enabled } : webhook)))
  }
    ...
    <div className="flex min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-[56rem]">
          ...
          <Card className="mb-6 overflow-hidden">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3 mb-4">
                ...
                <CardTitle className="text-xl">Push Notification Sound Playback Settings</CardTitle>
              </div>
              <CardDescription>
                Configure how sound notifications are played when the page monitor detects changes.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <CustomRadioGroup value={soundOption} onValueChange={setSoundOption} className="space-y-5">
                  ...
                  <div className="ml-9 mt-3 space-y-3">
                    ...
                    {audioUrlValidationStatus === "valid" && (
                      <div className="flex items-center gap-2 p-3 rounded-md bg-blue-50 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 text-sm border border-blue-100 dark:border-blue-900/30">
                        ...
                        <span>Audio URL has been validated and is ready to use.</span>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="mt-8 space-y-5">
                <div className="bg-gray-50 dark:bg-gray-900/40 rounded-lg p-4 border border-gray-100 dark:border-gray-800">
                  <div className="flex items-center justify-between mb-3">
                    <Label htmlFor="volume" className="text-base font-medium flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {volume[0] === 0 ? <Volume2 className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
                        <span className="sr-only">{volume[0] === 0 ? "Unmute" : "Mute"}</span>
                      </Button>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  {playbackMessage && (
                    <span
                      className={cn(
                        "text-sm",
                        playbackMessage.includes("disabled") ? "text-amber-600" : "text-green-600",
                      )}
                    >
                      {playbackMessage}
                    </span>
          ...
          <Card className="mb-6 overflow-hidden">
            <CardHeader className="pb-3">
              ...
              <CardDescription>Configure how long notification sounds should play when triggered.</CardDescription>
            </CardHeader>
            <CardContent>
              <CustomRadioGroup value={soundLength} onValueChange={setSoundLength} className="space-y-5">
                <div className="flex items-center space-x-3">
          ...
          <Card className="mb-6 overflow-hidden">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3 mb-4">
                ...
                <CardTitle className="text-xl">Email Alert Notifications</CardTitle>
              </div>
              <CardDescription>
                Receive email notifications when the page monitor detects changes on your monitored websites.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              {emailMessage.type === "info" && (
                <div className="mb-6 p-3 rounded-md bg-blue-50 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 text-sm border border-blue-100 dark:border-blue-900/30 flex items-start gap-2">
                  ...
                  <p>
                    {emailMessage.message} If you are not receiving OTPs, please refer to{" "}
                    <a
                      href="#"
              ...
              {!emailVerified ? (
                <div className="space-y-6">
                  {!showOtpInput ? (
                    ...
                    <div className="space-y-6">
                      <Label className="block">Enter the 6-digit verification code sent to your email</Label>
                  ...
                  <div className="relative overflow-hidden rounded-lg border border-green-200 dark:border-green-800 bg-gradient-to-r from-green-50 to-green-100/50 dark:from-green-900/20 dark:to-green-800/10 p-5">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-base font-medium text-green-800 dark:text-green-300">Email Verified</h4>
                          <p className="text-sm text-green-700 dark:text-green-400 font-medium">{email}</p>
                        </div>
                      </div>
          ...
          <Card className="mb-6 overflow-hidden">
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Configure webhook notifications to receive alerts when Page Monitor detects changes on your monitored
                websites.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="space-y-4 mb-6">
                {webhooks.map((webhook) => {
                  ...
                  const ServiceIcon = webhookServices.find((s) => s.name === webhook.service)?.icon || Globe
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:items-center justify-between p-4">
                        <div className="space-y-2 mb-3 md:mb-0">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-base">{webhook.name}</span>
                            {!webhook.enabled && (
                              <Badge
                                variant="outline"
                ...
                {webhooks.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-8 px-4 border border-dashed rounded-lg border-gray-300 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-1">No webhooks configured</p>
              ...
              {showWebhookForm ? (
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h4 className="font-medium text-lg">
                        {editingWebhook !== null ? "Edit Webhook" : "Add New Webhook"}
                      </h4>
                    </div>
                    {editingWebhook !== null && (
                      <Badge
                  ...
                  <div className="p-5 space-y-5">
                    ...
                    <div className="grid gap-3">
                      <Label htmlFor="webhook-service" className="text-sm font-medium">
                        Service Type
                      </Label>
                        ...
                        {webhookServices.map((service) => {
                          ...
                          return (
                            <button
                              key={service.id}
                              type="button"
                              onClick={() => handleServiceTagSelect(service.name)}
                              className={cn(
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        Select the service you want to connect to or choose "Custom URL" for a generic webhook
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3">
                      ...
                      <div className="space-y-2 max-w-lg">
                        <div className="relative">
                          <Input
                            ...
                            className={cn(
                              ...
                              urlValidationStatus === "invalid" && "border-red-500 focus-visible:ring-red-500",
                            )}
                          />
                        ...
                        {urlValidationStatus !== "idle" && urlValidationStatus !== "validating" && (
                          <div
                            className={cn(
                        ...
                        {webhookService !== "Custom URL" && (
                          <div className="flex items-start gap-2 p-3 rounded-md bg-blue-50 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 text-xs border border-blue-100 dark:border-blue-900/30">
                            ...
                            <div>
                              <p className="font-medium mb-1">Using {webhookService}</p>
                              <p>
                                ...
                                {webhookService === "Telegram" && "Enter your Telegram bot API URL with your chat ID."}
                                {webhookService === "Mattermost" &&
                                  "Use a Mattermost incoming webhook URL from your team settings."}
                                {webhookService === "Mobile Pushbullet" && "Enter your Pushbullet access token URL."}
                                {webhookService === "IFTTT" && "Use your IFTTT webhook URL with your maker key."}
                              </p>
                            </div>
                          </div>
                        )}
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800 rounded-b-lg">
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      {editingWebhook !== null ? "Update your webhook settings" : "Configure your new webhook"}
                    </div>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                      ...
                      <Button
                        ...
                        disabled={!webhookName || !webhookUrl || urlValidationStatus !== "valid"}
                        className={cn(
                          "transition-all",
                          !webhookName || !webhookUrl || urlValidationStatus !== "valid"
                            ? "opacity-70"
                ...
                <Button onClick={() => setShowWebhookForm(true)} className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Add New Webhook
                </Button>
              )}
        ...
        <ConfirmDialog
          ...
          description={`Are you sure you want to delete the webhook named '${webhooks.find((w) => w.id === webhookToDelete)?.name || ""}'? This action cannot be undone.`}
          confirmLabel="Delete"
          cancelLabel="Cancel"
