...
export default function HotkeysPage() {
  ...
  const [hotkeys, setHotkeys] = useState<Hotkey[]>([
    ...
    {
      id: "kill-notification-sound",
      label: "Kill Notification Sound",
      value: "",
    },
  ]);
  ...
  useEffect(() => {
    localStorage.setItem(
      "autoRefreshHotkeysEnabled",
      hotkeysEnabled.toString()
    );
  }, [hotkeysEnabled]);
  ...
  const handleHotkeyChange = (id: string, value: string) => {
    ...
    const duplicate = hotkeys.find(
      (hotkey) => hotkey.value === value && hotkey.id !== id
    );
    ...
    const updatedHotkeys = hotkeys.map((hotkey) =>
      hotkey.id === id ? { ...hotkey, value } : hotkey
    );
    ...
    toast({
      ...
      description: `Hotkey for "${
        hotkeys.find((h) => h.id === id)?.label
      }" has been updated.`,
    });
  };
        ...
        <div className='mx-auto max-w-[56rem]'>
          ...
          <Card className='mb-6 overflow-hidden'>
            <CardHeader className='flex flex-row items-start justify-between space-y-0 pb-3'>
              <div>
                ...
                <CardDescription>
                  Set Your Custom Keyboard Shortcuts for Auto Refresh Plus
                  Actions. Enable this option to use keyboard shortcuts for
            ...
            <CardContent>
              {!hotkeysEnabled && (
                <div className='bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 p-3 rounded-md text-sm border border-amber-100 dark:border-amber-900 flex items-start gap-2'>
                  ...
                  <p>
                    Hotkeys are currently disabled. Enable the toggle above to
          ...
          <Card className='mb-6 overflow-hidden'>
            <CardHeader className='pb-3'>
              ...
              <CardDescription>
                Configure keyboard shortcuts for common actions. Each shortcut
                must include at least one modifier key (Ctrl, Alt, Shift, or
                Command).
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    ...
                    <div className='md:col-span-2'>
                      <HotkeyInput
                        ...
                        value={hotkey.value}
                        onChange={(value) =>
                          handleHotkeyChange(hotkey.id, value)
                        }
            ...
            <CardContent>
              <ul className='space-y-2 text-sm text-muted-foreground'>
                <li className='flex items-start gap-2'>
                  ...
                  <span>
                    Use at least one modifier key (Ctrl, Alt, Shift, or Command)
                    with each hotkey.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <span className='text-green-600 dark:text-green-400'>•</span>
                  <span>
                    Avoid using hotkeys that conflict with browser or system
                    shortcuts.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <span className='text-green-600 dark:text-green-400'>•</span>
                  <span>
                    Click on any input field and press the desired key
                    combination to set a hotkey.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <span className='text-green-600 dark:text-green-400'>•</span>
                  <span>
                    Hotkeys are saved per device and will persist between
                    browser sessions.
                  </span>
                </li>
              </ul>
