...
import { AlertTriangle, PlayCircle, RefreshCw, Globe, HelpCircle, Lock, Upload, Download } from "lucide-react"
...
export default function AutostartPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (content) {
        ...
        const existingUrls = urlsValue.split(/\r?\n/).filter((line) => line.trim() !== "")
        ...
        toast({
          title: "URLs Imported",
          description: `Successfully imported ${lines.length} URL${lines.length !== 1 ? "s" : ""}.`,
          variant: "default",
        })
      }
    }
  ...
  const handleExportUrls = () => {
    ...
    toast({
      title: "URLs Exported",
      description: `Successfully exported ${urls.length} URL${urls.length !== 1 ? "s" : ""}.`,
      variant: "default",
    })
  }
          ...
          <Card className="mb-6 overflow-hidden">
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
              <div>
                ...
                <CardDescription>
                  Auto Refresh Plus will auto-start the page reload function when these URLs are opened in your browser.
                  It also opens all of these URLs when your browser restarts and begins auto-refreshing at the desired
                  interval.
                </CardDescription>
              </div>
              <Switch id="auto-start-urls" checked={autoStartUrls} onCheckedChange={setAutoStartUrls} />
            </CardHeader>
            <CardContent>
                  ...
                  <AccordionContent>
                    <ul className="list-disc pl-6 space-y-1 text-sm text-muted-foreground">
                      <li>
                        ...
                        <code className="bg-muted px-1 py-0.5 rounded">example.com</code>
                      </li>
                      <li>
                        To match a specific path, write{" "}
                        <code className="bg-muted px-1 py-0.5 rounded">example.com/path/to/page</code>
                      </li>
                      <li>
                        To match all pages under a path, write{" "}
                        <code className="bg-muted px-1 py-0.5 rounded">example.com/path/*</code>
                      </li>
                      <li>
                        To match a specific subdomain, write{" "}
                        <code className="bg-muted px-1 py-0.5 rounded">sub.example.com</code>
                      </li>
                      <li>
                        To match a specific subdomain with path, write{" "}
                        <code className="bg-muted px-1 py-0.5 rounded">sub.example.com/path</code>
                      </li>
                      <li>
...
blog.example.com/posts/*"
          ...
          <Card className={cn("mb-6 overflow-hidden transition-opacity duration-200", !autoStartUrls && "opacity-75")}>
              ...
              {!autoStartUrls && (
                <div className="absolute inset-0 bg-white/50 dark:bg-black/50 z-10 flex items-center justify-center">
                  <div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-md flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">Enable Auto Start URLs first</span>
                  </div>
                </div>
              )}
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <CardTitle className="text-xl">Refresh on Browser Restart</CardTitle>
                </div>
                <CardDescription>Autostart refreshing these URLs when the browser restarts.</CardDescription>
              </div>
              <Switch
                ...
                <div className="bg-gradient-to-r from-amber-50 to-amber-100 dark:from-amber-950/40 dark:to-amber-900/20 border-l-4 border-amber-400 dark:border-amber-500 rounded-md overflow-hidden shadow-sm">
                  <div className="p-4 flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <h4 className="font-medium text-amber-800 dark:text-amber-300">Wildcard URL Limitation</h4>
                      <p className="text-amber-700 dark:text-amber-400 text-sm">
                        Wildcard URLs (containing * character) are not supported on Browser restart. Only exact URLs
                        will be opened.
                      </p>
                    </div>
                  </div>
            ...
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3 relative">
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <CardTitle className="text-xl">Refresh on URLs Opening</CardTitle>
                </div>
                <CardDescription>Autostart Refresh When Any of These URLs Open</CardDescription>
              </div>
              <Switch
