...
export default function SubscriptionPage() {
  ...
  const [billingAddressDialogOpen, setBillingAddressDialogOpen] =
    useState(false);
  const [invoiceMissingInfoDialogOpen, setInvoiceMissingInfoDialogOpen] =
    useState(false);
  ...
  const [confirmDeletePaymentMethod, setConfirmDeletePaymentMethod] = useState<
    string | null
  >(null);
    ...
    if (success) {
      ...
      if (immediate) {
        ...
        toast({
          ...
          description:
            "Your subscription has been cancelled and your account has been downgraded to the Basic plan.",
        });
      } else {
  ...
  const handleConfirmDeletePaymentMethod = () => {
    if (confirmDeletePaymentMethod) {
      const methodToDelete = paymentMethods.find(
        (method) => method.id === confirmDeletePaymentMethod
      );

      // Remove the payment method
      setPaymentMethods((prevMethods) =>
        prevMethods.filter((method) => method.id !== confirmDeletePaymentMethod)
      );
  ...
  const getCardIcon = (brand: string) => {
    switch (brand.toLowerCase()) {
      ...
      case "mastercard":
        return (
          <div className='h-8 w-12 bg-red-500 rounded-md flex items-center justify-center text-white font-bold'>
            MC
          </div>
        );
      case "amex":
  ...
  const handleDownloadInvoice = async (invoiceId: string) => {
    try {
      ...
      endDate.setMonth(
        endDate.getMonth() + (subscription.billingPeriod === "yearly" ? 12 : 1)
      );
              ...
              <CardContent>
                ...
                {subscription.invoices.length === 0 && (
                  <div className='text-center py-8 text-gray-500 dark:text-gray-400'>
                    ...
                    <p>
                      Your payment history will appear here once you have made a
                      payment.
                    </p>
                  </div>
                )}
                    ...
                    <div className='px-5 py-4 flex items-center gap-4'>
                      ...
                      <div className='flex-1'>
                        ...
                        <p className='text-sm text-red-700/80 dark:text-red-400/90 mt-1'>
                          Please add a payment method to ensure uninterrupted
                          service for your subscription.
                        </p>
                      </div>
                    </div>
                ...
                <div className='mt-6 space-y-4'>
                  ...
                  <div className='p-4 border rounded-lg'>
                    <div className='flex items-start'>
                      ...
                      <div>
                        <p className='font-medium'>
                          {subscription.billingAddress.name}
                        </p>
        ...
        description={
          confirmDeletePaymentMethod
            ? `You're about to remove your saved payment method (${
                paymentMethods
                  .find((m) => m.id === confirmDeletePaymentMethod)
                  ?.brand.toUpperCase() || "Card"
              } •••• ${
                paymentMethods.find((m) => m.id === confirmDeletePaymentMethod)
                  ?.last4 || "****"
