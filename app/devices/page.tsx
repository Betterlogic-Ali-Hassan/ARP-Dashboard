...
import { useSubscriptionStore, useDeviceCountSync } from "@/lib/subscription-store"
...
export default function DevicesPage() {
  ...
  const enhancedDevices = devices.map((device) => ({
    ...
    browser: {
      ...
      version: `v${Math.floor(Math.random() * 120)}.${Math.floor(Math.random() * 10)}.${Math.floor(Math.random() * 10000)}`,
    },
  }))
  ...
  const usagePercentage = Math.round((devicesInUse / subscription.deviceLimit) * 100)
  ...
  const filteredDevices = enhancedDevices.filter((device) => {
    ...
    const matchesProfileFilter = !filterProfile || device.profileId === filterProfile
    const matchesStatusFilter = !filterStatus || device.status === filterStatus

    return matchesSearch && matchesTypeFilter && matchesProfileFilter && matchesStatusFilter
  })
  ...
  const handleDeviceSave = (deviceData: any) => {
    if (selectedDevice) {
      ...
      if (selectedDevice.profileId !== deviceData.profileId) {
        updateDevice(selectedDevice.id, { ...deviceData, profileId: undefined })
      ...
      // Assign the device to the selected profile
      if (deviceData.profileId) {
        assignDeviceToProfile(newDevice.id, deviceData.profileId)
      }

      toast({
        title: "Device added",
        description: "New device has been added successfully. Waiting for device to connect.",
      })
    }
    ...
    try {
      ...
      if (bulkDeleteSettings) {
        toast({
          ...
          description: `${selectedDevices.length} device${selectedDevices.length !== 1 ? "s" : ""} and their settings successfully reset and removed.`,
        })
      } else {
        toast({
          title: "Devices deleted",
          description: `${selectedDevices.length} device${selectedDevices.length !== 1 ? "s" : ""} successfully removed.`,
        })
      }
      ...
      toast({
        title: "Error",
        description: "There was a problem deleting the selected devices. Please try again.",
        variant: "destructive",
      })
    }
  }
      ...
      <Card className="mb-4 border-gray-200 dark:border-gray-800">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">{device.browser.version}</div>
            </div>
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem className="text-red-600 dark:text-red-400">Disconnect</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between text-sm mb-2">
            <div className="flex items-center">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">{device.ipAddress}</div>
              </div>
            </div>
            <div>
              <span className="font-medium">Status:</span> {getStatusBadge(device.status)}
            </div>
          </div>
          <div className="mt-2 flex items-center justify-between text-sm">
            ...
            <div>
              <span className="font-medium">Last Connected:</span> {device.lastConnected || "Never"}
            </div>
          </div>
        </div>
      </Card>
    ...
    <div className="flex min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-6xl">
          ...
          {isMobile && (
            <div className="flex items-center mb-4">
              <Button variant="ghost" size="icon" className="mr-2 -ml-2" onClick={() => setSidebarOpen(true)}>
                <Menu className="h-5 w-5" />
              </Button>
          ...
          <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-2xl text-sm md:text-base">
                Manage and sync your connected devices, with a maximum of {subscription.deviceLimit} devices allowed in
                your {subscription.plan === "team" ? "Team" : "Individual"} Plan.
                {subscription.status === "scheduled_downgrade" && (
                  <span className="block mt-1 text-amber-600 dark:text-amber-400">
                    Your plan will be downgraded on {subscription.nextBillingDate}.
                  </span>
                )}
              </p>
            </div>
              ...
              <div className="flex items-center gap-2 ml-4">
                <div
                  className={`h-2 w-2 rounded-full ${
                    syncStatus.inProgress ? "bg-amber-500 animate-pulse" : "bg-emerald-500"
                  }`}
                ></div>
            ...
            <div className="p-4 md:p-5">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent side="top" className="max-w-xs">
                            <p>All connected devices are counted toward your plan limit, including inactive ones.</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </h3>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium">{devicesInUse} devices in use</span>
                  <span className="text-gray-500 dark:text-gray-400">{devicesRemaining} remaining</span>
                </div>
                <Progress
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-4 gap-3">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {devicesRemaining > 0
                      ? `You can add ${devicesRemaining} more device${devicesRemaining !== 1 ? "s" : ""} with your current plan.`
          ...
          <div className="mb-6 flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="bg-white dark:bg-gray-900 h-11 flex-grow md:flex-grow-0">
                ...
                <DropdownMenuContent className="w-[calc(100vw-32px)] sm:w-56">
                  ...
                  <div className="p-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFilterProfile(value === "all" ? null : value)}
                    >
                      <SelectTrigger className="w-full">
          ...
          {filteredDevices.length > 0 ? (
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {filteredDevices.map((device) => {
                          ...
                          return (
                            <tr key={device.id} className="border-b border-gray-200 dark:border-gray-800">
                              ...
                              <td className="px-4 py-4 text-sm">{device.lastConnected || "Never"}</td>
                              <td className="px-4 py-4">{getStatusBadge(device.status)}</td>
            ...
            <div className="flex flex-col items-center justify-center py-8 md:py-12 text-center px-4">
              ...
              {(searchQuery || filterType || filterProfile || filterStatus) && (
                <Button variant="outline" onClick={clearFilters} className="h-10 w-full sm:w-auto">
                  Clear Filters
                </Button>
              )}
              {!searchQuery && !filterType && !filterProfile && !filterStatus && (
                <Button onClick={handleAddDevice} className="h-10 w-full sm:w-auto">
                  <Plus className="mr-2 h-4 w-4" />
        ...
        >
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setSelectedDevices([])} className="h-10">
              Cancel
            </Button>
            <Button variant="destructive" size="sm" onClick={handleBulkDelete} className="h-10">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to delete {selectedDevices.length} device{selectedDevices.length !== 1 ? "s" : ""}. This
          ...
          <div className="flex items-start space-x-2 py-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => setBulkDeleteSettings(checked === true)}
            />
            <div className="grid gap-1.5 leading-none">
              ...
              <p className="text-sm text-muted-foreground">
                This will remove all custom settings associated with these devices
              </p>
            </div>
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the device &quot;{selectedDevice?.name}&quot; and remove all associated data.
