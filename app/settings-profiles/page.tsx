...
export default function SettingsProfilesPage() {
  ...
  useEffect(() => {
    if (selectedProfile) {
      ...
      const availableProfiles = profiles.filter((p) => p.id !== selectedProfile.id && p.isDefault)
  ...
  const filteredProfiles = profiles.filter((profile) => {
    ...
    const isInactive =
      ...
      profileDevices.every((device) => device.status === "inactive" || device.status === "offline")
    ...
    if (selectedProfile) {
      ...
      if (associatedDevices.length > 0 && targetProfileId) {
        ...
        toast({
          ...
          description: `"${selectedProfile.name}" has been deleted and ${associatedDevices.length} device${associatedDevices.length !== 1 ? "s" : ""} reassigned to "${targetProfile?.name}".`,
        })
        ...
        if (defaultProfile && defaultProfile.id !== selectedProfile.id) {
          ...
          toast({
            title: "Profile deleted",
            description: `"${selectedProfile.name}" has been deleted and ${associatedDevices.length} device${associatedDevices.length !== 1 ? "s" : ""} reassigned to the default profile "${defaultProfile.name}".`,
          })
        } else {
          ...
          toast({
            title: "Profile deleted",
            description: `"${selectedProfile.name}" has been deleted and ${associatedDevices.length} device${associatedDevices.length !== 1 ? "s" : ""} have been unassigned.`,
          })
        }
      } else {
  ...
  const handleProfileSave = (profileData: any) => {
    ...
    const previouslyAssignedDevices = devices.filter((device) => device.profileId === profileId)
    const previouslyAssignedDeviceIds = previouslyAssignedDevices.map((device) => device.id)

    // Find devices that need to be assigned to this profile (newly selected)
    const devicesToAssign = selectedDeviceIds.filter((id) => !previouslyAssignedDeviceIds.includes(id))

    // Find devices that need to be unassigned from this profile (no longer selected)
    const devicesToUnassign = previouslyAssignedDeviceIds.filter((id) => !selectedDeviceIds.includes(id))
  ...
  const handleDeviceAssociationSave = (deviceIds: string[]) => {
    ...
    // Find devices that need to be assigned to this profile (newly selected)
    const devicesToAssign = deviceIds.filter((id) => !previouslyAssignedDeviceIds.includes(id))

    // Find devices that need to be unassigned from this profile (no longer selected)
    const devicesToUnassign = previouslyAssignedDeviceIds.filter((id) => !deviceIds.includes(id))
  ...
  const getDeviceSyncStats = (profileId: string) => {
    const associatedDevices = devices.filter((device) => device.profileId === profileId)
  ...
  const getDeviceTypeCounts = (profileId: string) => {
    ...
    associatedDevices.forEach((device) => {
      ...
      const model = device.model || "Generic"
    ...
    for (const type in deviceModels) {
      ...
      if (Object.keys(models).length === 1 && Object.keys(models)[0] === "Generic") {
  ...
  const getInactivityReason = (profileId: string) => {
    ...
    } else if (profileDevices.every((device) => device.status === "inactive" || device.status === "offline")) {
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="flex items-center">
                <h1 className="text-3xl font-bold tracking-tight mb-2">Settings Profiles</h1>
                <a
                ...
                <div className="flex items-center gap-2 ml-4">
                  <div
                    className={`h-2 w-2 rounded-full ${syncStatus.inProgress ? "bg-amber-500 animate-pulse" : "bg-emerald-500"}`}
                  ></div>
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-2xl">
                Create and manage settings profiles to apply consistent configurations across multiple devices.
              </p>
            </div>
            <Button
          ...
          <Tabs defaultValue="all" className="mb-8" value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="mb-4">
              <TabsTrigger value="all" className={activeTab === "all" ? "font-medium" : ""}>
                All Profiles
              </TabsTrigger>
              <TabsTrigger value="active" className={activeTab === "active" ? "font-medium" : ""}>
                Active
              </TabsTrigger>
              <TabsTrigger value="inactive" className={activeTab === "inactive" ? "font-medium" : ""}>
                Inactive
              </TabsTrigger>
            </TabsList>
              ...
              <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {filteredProfiles.map((profile) => {
                  ...
                  return (
                    <Card
                      key={profile.id}
                      className={isDefault ? "border-2 border-emerald-100 dark:border-emerald-900" : ""}
                    >
                      ...
                      <CardContent className="pb-4">
                        <div className="mb-4">
                          ...
                          {deviceTags.length > 0 ? (
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">No devices associated</p>
                          )}
                        </div>
                        <div>
                          <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                          ...
                          {syncStats.total > 0 ? (
                            <div className="space-y-3">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="font-medium text-sm">{syncStats.syncing} devices</span>
                              </div>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium text-sm">{syncStats.inactive} devices</span>
                              </div>
                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                                {syncStats.total > 0 && (
                                  <div
                                    className="bg-emerald-500 h-2 rounded-full"
                                    style={{ width: `${(syncStats.syncing / syncStats.total) * 100}%` }}
                                  ></div>
                                )}
                              </div>
                            </div>
                          ) : (
                            <p className="text-sm text-gray-500 dark:text-gray-400">No devices to sync</p>
                          )}
                        </div>
                      </CardContent>
                      <CardFooter className="flex flex-col sm:flex-row justify-between gap-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => handleManageAssociations(profile)}>
                            ...
                            <DropdownMenuItem onClick={() => handleExportSettings(profile)}>
                              <Download className="mr-2 h-4 w-4" />
                              <span>Export Settings</span>
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleImportSettings(profile)}>
                              <Upload className="mr-2 h-4 w-4" />
                              <span>Import Settings</span>
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleSyncProfiles()}>
            ...
            <TabsContent value="inactive" className="mt-0">
              {filteredProfiles.length > 0 ? (
                        ...
                        <CardHeader className="pb-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              {inactivityReason && (
                                <span className="text-xs text-gray-500 mt-1">{inactivityReason}</span>
                              )}
                            </div>
                          </div>
                          <CardDescription className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">No Inactive Profiles</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md">
                    You don't have any inactive profiles. A profile becomes inactive when it has no associated devices
                    or all its devices are offline.
                  </p>
                </div>
              )}
            </TabsContent>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          {selectedProfile && (
            <div className="py-2">
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                Are you sure you want to delete the profile &quot;{selectedProfile.name}&quot;? This action cannot be
                undone.
              </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-sm">
                        {getDevicesByProfileId(selectedProfile.id).length} device
                        {getDevicesByProfileId(selectedProfile.id).length !== 1 ? "s" : ""} associated with this profile
                      </h4>
                      ...
                      {getAvailableProfiles().length > 0 ? (
                        ...
                        <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 rounded-md text-sm">
                          ...
                          <div>No other profiles available. Devices will be unassigned.</div>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDeleteProfile} className="bg-red-600 hover:bg-red-700 text-white">
              Delete Profile
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
