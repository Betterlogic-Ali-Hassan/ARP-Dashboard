...
export default function SynchronizingPage() {
  ...
  const [syncStatus, setSyncStatus] = useState<"idle" | "success" | "waiting" | "error">("idle")
  ...
  const [currentPlan, setCurrentPlan] = useState<"basic" | "individual" | "team" | "enterprise">("team")
  ...
  const validateDeviceName = (name: string): boolean => {
    ...
    if (!/^[a-zA-Z0-9\s\-_]+$/.test(name)) {
      setDeviceNameError("Device name can only contain letters, numbers, spaces, hyphens, and underscores")
      return false
    }
  ...
  const handleSync = () => {
    ...
    if (syncKey.length < 29) {
      ...
      toast({
        ...
        description: "Please enter a complete sync key in the format XXXXX-XXXXX-XXXXX-XXXXX-XXXXX",
        variant: "destructive",
      })
      return
    }
    ...
    setTimeout(() => {
      ...
      if (syncKey === "AP349-SCI2Q-H6DO2-NPHBS-916H2") {
        ...
        toast({
          ...
          description: "Your device has been successfully synchronized with the Default Profile",
          variant: "default",
        })
        ...
        toast({
          ...
          description: "Your device is waiting for a settings profile to be assigned",
          variant: "default",
        })
      } else {
        setSyncStatus("error")
  ...
  return (
    <div className="flex min-h-screen">
      <Sidebar activePage="synchronizing" />
      <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 bg-background">
        <div className="w-full max-w-4xl mx-auto">
          {/* Page header */}
          <div className="mb-10 text-center">
            <h1 className="text-4xl font-bold tracking-tight mb-3">Device Synchronization</h1>
            <p className="text-muted-foreground max-w-lg mx-auto">
              Connect and synchronize your devices to share settings and preferences across your account
            </p>
          </div>
          ...
          <div className="mb-8">
            <Tabs value={syncMode} onValueChange={setSyncMode} className="w-full max-w-[40rem] mx-auto">
              {/* Only show the tabs when not logged in and not synced */}
              {!isLoggedIn && syncStatus !== "success" && syncStatus !== "waiting" && (
              ...
              <TabsContent value="primary" className="space-y-4">
                  ...
                  <Card className="border-primary/20">
                    <CardContent className="pt-6">
                      <div className="text-center mb-6">
                        ...
                        <h2 className="text-2xl font-bold mb-2">You are logged in as the Primary Device</h2>
                        <p className="text-muted-foreground">
                          Total <span className="font-semibold">{totalDevices} devices</span> attached to your account
                          under <span className="font-semibold">{totalProfiles} profiles</span>
                        </p>
                      </div>
                      ...
                      <div className="mt-8 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-md p-4">
                        ...
                        <ul className="space-y-2 text-sm text-blue-700 dark:text-blue-300">
                          <li className="flex items-start">
                            ...
                            <span>Your settings are being synchronized to all connected devices</span>
                          </li>
                          <li className="flex items-start">
                            <span className="mr-2">•</span>
                            <span>You can manage all connected devices from the Devices page</span>
                          </li>
                          <li className="flex items-start">
                            <span className="mr-2">•</span>
                            <span>Any changes you make will automatically sync to secondary devices</span>
                          </li>
                        </ul>
                      </div>
                    </CardContent>
                    ...
                    <CardContent>
                      {showOtpVerification ? (
                        ...
                        <form onSubmit={handleVerifyOtp} className="space-y-4">
                          ...
                          <Button type="submit" className="w-full" disabled={isLoggingIn || otp.length !== 6}>
                            {isLoggingIn ? (
                              <>
                          ...
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                otpResendDisabled && "text-muted-foreground cursor-not-allowed hover:no-underline",
                              )}
                              disabled={isLoggingIn || otpResendDisabled}
                            >
                              {otpResendDisabled ? `Resend code (${otpResendCountdown}s)` : "Resend code"}
                            </button>
                          </div>
                        ...
                        <form onSubmit={isSignUp ? handleSignUp : handlePrimaryAuth} className="space-y-4">
                          {isSignUp && (
                            <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="email" className="text-sm font-medium flex items-center justify-between">
                              Email address
                              {email && !/^\s*$/.test(email) && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) === false && (
                          ...
                          <Button type="submit" className="w-full" disabled={isLoggingIn}>
                            {isLoggingIn ? (
                              ...
                              <>
                                <ArrowRight className="mr-2 h-4 w-4" />
                                {isSignUp ? "Create account" : "Log in and continue"}
                              </>
                            )}
                          </Button>
                          ...
                          <div className="mt-4 space-y-2">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                toast({
                                  title: isSignUp ? "Google Sign Up" : "Google Login",
                                  description: `Google authentication would be triggered here for ${isSignUp ? "sign up" : "login"}`,
                                })
                              }}
                            >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                toast({
                                  title: isSignUp ? "Microsoft Sign Up" : "Microsoft Login",
                                  description: `Microsoft authentication would be triggered here for ${isSignUp ? "sign up" : "login"}`,
                                })
                              }}
                            >
                              <svg className="mr-2 h-4 w-4" aria-hidden="true" focusable="false" viewBox="0 0 23 23">
                              ...
                              {isSignUp ? "Continue with Microsoft" : "Login with Microsoft"}
                            </Button>
                          </div>
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg">What is a Primary (Master) Device?</CardTitle>
                  </CardHeader>
                  <CardContent className="text-sm text-muted-foreground space-y-2">
                    <p>The Primary Device is your main control device. It is responsible for:</p>
                    <p>• Creating and managing the settings profiles</p>
                    <p>• Syncing those settings across all connected secondary devices</p>
                    <p>• Making real-time updates that reflect automatically on slave devices</p>
                  </CardContent>
              ...
              <TabsContent value="secondary" className="space-y-4">
                <Card className="border-primary/20">
                  <CardHeader>
                    ...
                    <CardDescription>Connect this device to your primary device using a sync key</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-6">
                      {/* Device Name Input */}
                      <div className="space-y-2">
                        <Label htmlFor="deviceName" className="flex items-center justify-between">
                          Device Name
                          <span className="text-xs text-muted-foreground">{deviceName.length}/30</span>
                        </Label>
                        <Input
                          id="deviceName"
                          value={deviceName}
                          onChange={(e) => {
                            ...
                            if (deviceNameError) validateDeviceName(e.target.value)
                          }}
                          placeholder="Enter a name for this device"
                          className={cn(
                            "transition-all duration-200",
                            deviceNameError ? "border-red-500 focus-visible:ring-red-500" : "",
                          )}
                          disabled={isSyncing || syncStatus === "success" || syncStatus === "waiting"}
                        />
                        {deviceNameError ? (
                          <p className="text-red-500 text-xs flex items-center mt-1">
                            <AlertCircle className="h-3 w-3 mr-1" /> {deviceNameError}
                          </p>
                        ) : (
                          <p className="text-xs text-muted-foreground">
                            Choose a name that helps you identify this device (e.g., "Work Laptop", "Home PC")
                          </p>
                        )}
                      </div>
                      ...
                      {syncStatus === "idle" && !isSyncing && (
                        <Button onClick={handleSync} className="w-full" disabled={syncKey.length < 29}>
                          Connect Device
                        </Button>
                      )}
                            ...
                            <div className="flex flex-col">
                              ...
                              <div className="space-y-3">
                                <p className="text-sm text-amber-800 dark:text-amber-300">
                                  Your <strong>{PLANS[currentPlan].name} Plan</strong> has a limit of{" "}
                                  <strong>{PLANS[currentPlan].deviceLimit} devices</strong>, and you've reached this
                                  limit.
                                </p>
                                <div className="bg-amber-100 dark:bg-amber-900/50 rounded-md p-3">
                                  ...
                                  <ul className="text-xs text-amber-700 dark:text-amber-400 space-y-2">
                                    <li className="flex items-start">
                                      <span className="mr-2">•</span>
                                      <span>Remove inactive devices from your account</span>
                                    </li>
                                    <li className="flex items-start">
                                      <span className="mr-2">•</span>
                                      <span>
                      ...
                      {syncStatus === "success" && (
                        ...
                        >
                          ...
                          <div className="text-center">
                            ...
                            <p className="text-muted-foreground">
                              Your device "{deviceName}" is now connected to your account
                            </p>
                          </div>
                          ...
                          <div className="bg-card border rounded-lg p-4 shadow-sm">
                            <div className="space-y-4">
                              <div>
                                ...
                                <div className="bg-muted rounded-md px-3 py-2 font-medium">{deviceName}</div>
                              </div>
                          ...
                          <div className="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-md p-4">
                            ...
                            <ul className="space-y-2 text-sm text-green-700 dark:text-green-300">
                              <li className="flex items-start">
                                <span className="mr-2">•</span>
                                <span>Your settings are now being synchronized</span>
                              </li>
                              <li className="flex items-start">
                                <span className="mr-2">•</span>
                                <span>Any changes made on your primary device will automatically apply here</span>
                              </li>
                              <li className="flex items-start">
                                <span className="mr-2">•</span>
                                <span>You can manage this device from your primary device's dashboard</span>
                              </li>
                            </ul>
                          </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <div className="w-full bg-amber-100 dark:bg-amber-900/50 rounded-md p-3 mb-3">
                                <div className="flex items-center">
                                  ...
                                  <p className="text-sm text-amber-800 dark:text-amber-300">
                                    Your device is in queue for administrator approval
                                  </p>
                                </div>
                              </div>
                              <div className="text-xs text-amber-700 dark:text-amber-400 space-y-2 w-full">
                                <p className="flex items-start">
                                  <span className="mr-2">•</span>
                                  <span>Your device has been registered successfully</span>
                                </p>
                                <p className="flex items-start">
                                  <span className="mr-2">•</span>
                                  <span>An administrator needs to assign a settings profile</span>
                                </p>
                                <p className="flex items-start">
                                  <span className="mr-2">•</span>
                                  <span>You'll receive a notification when your device is ready</span>
                                </p>
                              </div>
                            </div>
                          </div>
                      ...
                      {syncStatus === "error" && (
                        ...
                        >
                          ...
                          <div className="p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-md text-center text-red-800 dark:text-red-300">
                            ...
                            <p className="text-sm">
                              The sync key you entered is invalid or has expired. Please check the key and try again.
                            </p>
                          </div>
                          <Button
                            variant="outline"
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg">What is a Secondary (Slave) Device?</CardTitle>
                  </CardHeader>
                  <CardContent className="text-sm text-muted-foreground space-y-2">
                    <p>The Secondary Device is a linked device that mirrors the settings of the primary device. It:</p>
                    <p>• Uses the Sync Key to connect to the primary device</p>
                    <p>• Automatically receives all updates made on the master</p>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="pt-2">
              By signing out, your Auto Refresh Plus extension will be disconnected from the cloud sync system.
            </DialogDescription>
            <div className="text-sm text-muted-foreground mt-2">
              The slave devices will no longer sync settings from your device, but the settings they already have will
              remain unchanged.
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => setLogoutConfirmed(checked === true)}
              />
              <label
            ...
            <div className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => setDisconnectAllDevices(checked === true)}
              />
              <label
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleLogoutConfirm} disabled={!logoutConfirmed}>
              Confirm Logout
            </Button>
          </DialogFooter>
        </DialogContent>
